...
- 본문은 기술명보다 한 일이 먼저 읽히게 씁니다. 예를 들어 `holdout`은 `학습에 쓰지 않은 질문`으로 풀어 씁니다.
- `LoRA`, `vLLM`, `MVI`, `MVVM`, `Room`, `AlarmManager`처럼 경력을 증명하는 표준 기술명은 기술 태그나 상세 설명에 유지합니다.
- `평가 하네스`처럼 포트폴리오에서 강조할 용어는 첫 등장에 `같은 조건으로 모델을 비교하는 도구`처럼 뜻을 함께 적습니다.
- `Clean holdout`, `LLM as a Judge`, `QA Board`처럼 팀마다 다르게 쓰거나 설명 없이 이해하기 어려운 표현은 사용하지 않습니다.
- 프로젝트 성과에는 사용자가 체감한 변화, 팀이 얻은 이점, 검증된 수치 가운데 하나를 포함합니다.
- 상승률은 비교 전후 점수에서 직접 계산하고 표본 수와 평가 조건을 함께 적습니다. 회귀가 확인된 실험은 개선 수치만 떼어 쓰지 않습니다.
- 사내 코드명, 비공개 제품명과 내부 약어는 공개 문구에서 제외합니다.
