...
- 소개: 리멤버 프로필 소개(NextStep `career/resumes/remember-profile-base-2026-08-21.md`)의 첫 문단만 그대로 쓴다. 회사별 상세 문단 두 개는 경력, 주요 프로젝트와 내용이 겹쳐 2026-08-23 제거했다. "경계를 먼저 나누고 만듭니다" 같은 추상 원칙 제목은 처음 보는 사람이 이해하지 못해 2026-08-22 금지했다. 콘텐츠는 `src/data/about.ts`.
...
- 좌측에 토스식 섹션 레일(`SectionRail.tsx`)을 둔다. 활성 섹션의 선이 길어지고 라벨이 나타나며 1360px 이하에서는 숨긴다 (2026-08-23). 첫 화면에서는 히어로와 겹쳐 보여 소개부터 나타나고, 상단 내비게이션도 같은 훅(`useActiveSection`)으로 현재 섹션에 밑줄을 켠다.
- 첫 화면에서 소개, 경력 구간에 들어오면 `App.tsx`의 IntersectionObserver가 main data-zone을 바꿔 배경을 paper에서 paper-2로 전환한다 (2026-08-23).
- 소개는 리드 한 문단만 남았다(1.05~1.2rem). 전체 글자 크기는 루트 font-size 93.75%(15px 기준)로 한 단계 줄였다 (2026-08-23).
